...
- **`grok-codebase`** — a predict-then-verify reading partner for unfamiliar code: instead
  of summarizing the code for you, it makes you *predict* what a spot does before revealing
  the real code (so the gap teaches you), and *explain back* the core spots in your own
  words — so the mental model lodges in your head instead of evaporating with a summary.
