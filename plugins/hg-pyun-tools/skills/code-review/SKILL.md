---
name: code-review
description: Severity-rated review of the current diff or a named file. Delegates to the reviewer agent and produces CRITICAL/MAJOR/MINOR findings with file:line evidence. TRIGGER when user says "review my changes", "코드 리뷰 해줘", "/code-review", or asks for severity-rated feedback. DO NOT TRIGGER for general code questions or syntax help.
---

<Purpose>
Produce a severity-rated review of the current diff (or a specified file) using the `reviewer` agent. Returns CRITICAL/MAJOR/MINOR findings each anchored to file:line, with a summary count.
</Purpose>
...
3. **Delegate to the reviewer agent via Task tool:**
   ```
   Task(
   ...
   ```
4. **Present the result.** Use the reviewer's Output_Format verbatim.
</Steps>

<Tool_Usage>
- Bash for `git diff`/`git status` (no mutations)
- Read for named-file reviews
- Task tool with `subagent_type="reviewer"` for delegation
</Tool_Usage>

<Examples>
**Example 1 — staged diff review:**
User: "review my staged changes"
Steps: `git diff --cached` → delegate to `reviewer` → present findings.

**Example 2 — named file review:**
User: "code-review src/auth/login.ts"
Steps: Read the file → delegate to `reviewer` with file contents → present findings.
</Examples>

<Final_Checklist>
- Did I identify the review surface before delegating?
- Did I delegate to `reviewer` (not inline)?
